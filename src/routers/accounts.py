...
from src.views.transactions import TransactionListView
...
async def get_account(user: T_User, session: T_Session):
    account = await account_service.get_accounts(user.user_cpf, session)

    return {"accounts": account}


@router.get("/extrato/{banco_id}", response_model=TransactionListView)
async def get_extract(banco_id: int, session: T_Session, current_user: T_User):
    extract = await account_service.get_extract(
