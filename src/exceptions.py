...
class NotFoundError(AppExceptions):
    def __init__(
        self, status_code=HTTPStatus.NOT_FOUND, detail="Not found error."
    ):
...
class ConflictError(AppExceptions):
    def __init__(
        self, status_code=HTTPStatus.CONFLICT, detail="Conflic error."
    ):
        super().__init__(status_code, detail)
...
class ExpiredTokenError(AppExceptions):
    def __init__(
        self, status_code=HTTPStatus.UNAUTHORIZED, detail="Token was expired."
    ):
        super().__init__(status_code, detail)
